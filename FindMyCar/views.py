...
def display_cars(request):
    ...
    dist = abs(int(request.POST["distance"]))
    # Reuse the Pre-Data load csv loads and determine lat and long of the user provided zip code
    # Make the page behave as per the zip code's availability
    if pin in data.zip_Info["zip_code"].values and dist > 1:
        ...
        if len(zip_values.values) > 0:
            ...
            if len(details) > 0:
                ...
                return render(request, 'test.html', {'details': details, 'no_zip_dist_detail': (False, False, False)})
            else:
                conn.close()
                return render(request, 'test.html', {'no_zip_dist_detail': (False, False, True)})
        else:
            conn.close()
            return render(request, 'test.html', {'no_zip_dist_detail': (False, True, False)})

    else:
        return render(request, 'test.html', {'no_zip_dist_detail': (True, False, False)})
